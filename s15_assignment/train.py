...
def run_validation(model, validation_ds, tokenizer_src, tokenizer_tgt, max_len, device, print_msg, global_step, writer, num_examples=2):
    # Run validation on the model and compute evaluation metrics
    model.eval()
    count = 0
...
def get_or_build_tokenizer(config, ds, lang):
    ...
    if not Path.exists(tokenizer_path):
        # Build the tokenizer if it doesn't exist
...
def get_ds(config):
    ...
    # Find the maximum length of sentences in the source and target languages
    max_len_src = 0
    max_len_tgt = 0
